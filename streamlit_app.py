...
if  st.session_state.select == 'Utama':
    ...
    def seatgen(subclass):
        ...
        def chair_formatter(label,t):
            if t == "L":
                return f"""[<span class="kursi" style='color: #006aff;margin: 0;'>{label}</span>]"""
            if t == "P":
                return f"""[<span class="kursi" style='color: #e91ef7'>{label}</span>]"""
            return f"""[<span class="kursi" style='color: gray'>{label}</span>]"""
            ...
            st.markdown(f"""
                        ...
                        @media (max-width: 470px) and (min-resolution: 2dppx){{
                            ...
                            .kursi {{
                                letter-spacing:-0.07rem;
                                padding:1px .1px;
                                font-size: {0.58 if subclass == "X-5" or subclass == "X-6" else 0.65}rem;
                                line-height: -1.9rem;
                                display: flex;
                                justify-content: space-between;
                                margin: 0;
